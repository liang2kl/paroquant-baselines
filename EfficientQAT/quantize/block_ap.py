...
def update_dataset(layer, dataset, dev, **kwargs):
    with torch.no_grad():
...
def block_ap(
    ...
    for block_index in range(len(layers)):
        ...
        set_quant_state(
            qlayer, weight_quant=False
        )  # deactivate quantization for obtaining ground truth
        if args.epochs > 0:
            update_dataset(
                qlayer, fp_train_inps, dev, attention_mask=attention_mask, **kwargs
            )
            update_dataset(
                qlayer, fp_val_inps, dev, attention_mask=attention_mask, **kwargs
            )
        ...
        if args.epochs > 0:
            ...
            if args.weight_lr > 0:
                ...
                param.append(
                    {"params": weight_parameters(qlayer), "lr": args.weight_lr}
                )
                empty_optimizer_2 = torch.optim.AdamW(
                    [torch.tensor(0)], lr=args.weight_lr
                )
            ...
            for epoch in range(args.epochs):
                ...
                for index, (quant_inps, fp_inps) in enumerate(
                    zip(quant_train_inps, fp_train_inps)
                ):
                    ...
                    if args.quant_lr > 0:
                        ...
                        optimizer.param_groups[quant_index][
                            "lr"
                        ] = quant_scheduler.get_lr()[0]
                    if args.weight_lr > 0:
                        weight_scheduler.step()
                        optimizer.param_groups[weight_index][
                            "lr"
                        ] = weight_scheduler.get_lr()[0]

                # step 6.5: calculate validation loss
                val_loss_list = []
                for index, (quant_inps, fp_inps) in enumerate(
                    zip(quant_val_inps, fp_val_inps)
                ):
                    # obtain output of quantization model
                    with torch.no_grad():
                        with torch.cuda.amp.autocast():
        ...
        if args.epochs > 0:
            update_dataset(
                qlayer, quant_train_inps, dev, attention_mask=attention_mask, **kwargs
            )
            update_dataset(
                qlayer, quant_val_inps, dev, attention_mask=attention_mask, **kwargs
            )
