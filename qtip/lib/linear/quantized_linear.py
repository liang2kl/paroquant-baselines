...
class QuantizedLinear(nn.Module):
    ...
    def to_pseudo_fp16(self) -> nn.Linear:
        linear = nn.Linear(
            self.in_features,
            self.out_features,
        ...
        identity = torch.eye(
            self.in_features,
            device=self.SU.device,  # Ensure identity is on the same device as the module's buffers
            dtype=self.dtype
        )
        ...
        with torch.no_grad():
            transposed_weight = self.no_ckpt_forward(identity)

        self.bias = original_bias

        linear.weight.data = transposed_weight.T
        if self.bias is not None:
            linear.bias.data = self.bias.clone().to(dtype=self.dtype)

        return linear.to(self.SU.device)