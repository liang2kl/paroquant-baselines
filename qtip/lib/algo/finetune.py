...
def quantize_finetune_decoder_layer(mixed_layer, quant_order, idx, cb, args,
    ...
    print(f"quantize_finetune_decoder_layer {idx} on device {device}")
    ...
    for quant_i, (linear_attr, name, in_hess_name, out_hess_name,
        ...
        print(f'quantizing layer {idx} {name}')
        utils.clean()
...
def infer(args, end_dev, n_layers, in_q, out_q):
    with torch.no_grad():
        ...
        while True:
            data = in_q.get()
            print(f'infer got data: {data is not None}')
            if data is None:
                return
            out_q.put(
                model(data.to(0))['logits'][:, :-1].contiguous().softmax(
                    dim=-1).cpu())
            print('infer put data')

def calculate_ce_loss_model(model, target_model, dataloader, start_dev):
    model.eval()
    total_loss = 0
    ct = 0
    with torch.no_grad():
        for source, _ in dataloader:
            output = model(source.to(0))['logits'][:, :-1].contiguous()
            output = output.view(-1, output.shape[-1])
            target = target_model(
                source.to(0))['logits'][:, :-1].contiguous().softmax(
...
def finetune_susv_e2e(quant_model, start_dev, devset, orig_dtype, args):
    ...
    with torch.no_grad():
        n_layers = len(quant_model.model.layers)
        fake_dev_map = {
            'model.embed_tokens': 0,
            'model.rotary_emb': 0,
            'model.norm': start_dev - 1,
            'lm_head': start_dev - 1
        }
        per_dev = math.ceil(n_layers / start_dev)
        for i in range(n_layers):
            fake_dev_map[f'model.layers.{i}'] = (i + 1) // per_dev

        target_model = Qwen3ForCausalLMFP16.from_pretrained(args.base_model,
                                                            torch_dtype='auto',
                                                            device_map=fake_dev_map,
                                                            low_cpu_mem_usage=True)
    ...
    best_loss = calculate_ce_loss_model(quant_model, target_model, valid_dl,
                                        start_dev)
    ...
    for epoch in range(args.ft_epochs):
        for bidx, (source, _) in enumerate(train_dl):
            with torch.autocast(device_type='cuda',
                                ...
                                enabled=True):
                output = quant_model(
                    source.to(0))['logits'][:, :-1].contiguous()
                with torch.no_grad():
                    target = target_model(
                        source.to(0)
                    )['logits'][:, :-1].contiguous().softmax(dim=-1).to(
                        output.device)
                target = target.view(-1, target.shape[-1])
        ...
        if epoch % args.ft_valid_freq == (args.ft_valid_freq - 1):
            test_loss = calculate_ce_loss_model(quant_model, target_model,
                                                valid_dl, start_dev)
            if test_loss < best_loss:
                glog.info(
